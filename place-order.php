...
//OrderMasterQuery
$orderq = mysqli_query($connection, "insert into tbl_ordermaster(order_date,user_id,order_status) values('{$date}','{$userid}','Pending')") or die(mysqli_error($connection));
//It Will Store Last Record Inserted Value/ID
$orderid = mysqli_insert_id($connection);
//echo $orderid;

$shippingq = mysqli_query($connection, " update tbl_ordermaster set shipping_name='{$name}',shipping_mobile='{$mobile}',shipping_address='{$address}' where order_id ='{$orderid}' ") or die(mysqli_error($connection));
...
foreach ($_SESSION['productcart'] as $key => $value) {
    ...
    $orderdetailq = mysqli_query($connection, "insert into tbl_orderdetails (order_id,product_id,order_quantity,order_price) values('{$orderid}','{$value}','{$_SESSION['qtycart'][$key]}','{$productdata['product_price']}')") or die(mysqli_error($connection));
}
	...
	<div class="welcome">
		<div class="container">
			...
			<h3 class="tittle-w3l">
                            ...
                            <br/>
                            Make Payment Via you get Product Delivery 
